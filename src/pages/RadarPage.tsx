    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={handleBackdrop}>
      <div className="relative mx-4 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card shadow-2xl">
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 rounded-t-2xl border-b bg-card px-6 py-4">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-1 text-base font-bold text-foreground">
              {item.pdm_descricao || (item.descricao_item ? item.descricao_item.slice(0, 60) + (item.descricao_item.length > 60 ? "…" : "") : "Item sem descrição")}
            </h3>
        ...
        <div className="space-y-6 px-6 py-5">
          ...
          {item.descricao_item && (
            <div>
              <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Descrição Completa</p>
              <div className="max-h-36 overflow-y-auto rounded-lg border bg-muted/30 p-3 text-xs text-foreground leading-relaxed">
                {item.descricao_item}
              </div>
            </div>
          )}
    ...
    <div className={compact ? "mb-4" : "mb-6"}>
      <div className="rounded-xl border bg-card">
        ...
        >
          ...
          <motion.span animate={{ rotate: showFiltros ? 180 : 0 }} transition={{ duration: 0.25 }}>
            <ChevronDown size={16} />
          </motion.span>
        </button>
